...
class AlumnoDetailScreen extends StatefulWidget {
  ...
  @override
  State<AlumnoDetailScreen> createState() => _S();
}
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Tel: ${a.telefono}'),
            Text(
              'Vencimiento: ${DateFormat('dd/MM/yyyy').format(a.fechaVencimiento)} - ${a.estado}',
            ),
            Text(
              'Nacimiento: ${a.fechaNacimiento != null ? DateFormat('dd/MM/yyyy').format(DateTime.parse(a.fechaNacimiento!)) : '-'} (${a.edadTexto})',
            ),
            Text('Monto: \$${a.monto}'),
            ...
            Row(
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () async {
                    final txt =
                        'Hola ${a.nombre}! Te habla Central Boxing. Tu cuota vence el ${DateFormat('dd/MM/yyyy').format(a.fechaVencimiento)} Monto \$${a.monto}. Te esperamos!';
            ...
            Expanded(
              child: FutureBuilder(
                ...
                builder: (_, snap) {
                  ...
                  if (list.isEmpty) return const Text('Sin pagos');
                  return ListView.builder(
                    itemCount: list.length,
                    itemBuilder: (_, i) {
                      final p = list[i];
                      return ListTile(
                        title: Text('\$${p.monto} - ${p.mesCubierto}'),
                        subtitle: Text(
                          '${DateFormat('dd/MM/yyyy').format(p.fechaPago)} ${p.metodo ?? ''}',
                        ),
                      );
                    },
                  );
