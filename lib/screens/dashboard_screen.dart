  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: CustomScrollView(slivers: [
        ...
        SliverToBoxAdapter(child: SingleChildScrollView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal:8), child: Row(children: ['todos','al_dia','por_vencer','vencido'].map((f)=> Padding(padding: const EdgeInsets.only(right:6), child: FilterChip(label: Text(f.replaceAll('_',' '), style: const TextStyle(fontSize:12)), selected: p.filtro==f, onSelected:(_){p.filtro=f; p.load();}, selectedColor: const Color(0xFFD32F2F).withValues(alpha:0.2)))).toList()))),
        SliverList.builder(itemCount: p.alumnos.length, itemBuilder: (_, i){ final a=p.alumnos[i]; return Dismissible(key: ValueKey(a.id), background: Container(color: Colors.green, alignment: Alignment.centerLeft, padding: const EdgeInsets.only(left:16), child: const Icon(Icons.message, color: Colors.white)), direction: a.estado=='vencido'||a.estado=='por_vencer'?DismissDirection.startToEnd:DismissDirection.none, confirmDismiss: (_) async { final wa=a.telefono.replaceAll(RegExp(r'[^0-9]'),''); final fecha=DateFormat('dd/MM/yyyy','es').format(a.fechaVencimiento); final vencido=a.estado=='vencido'; final txt=NotificationService.buildWhatsAppMessage(a.nombre, fecha, a.monto.toString(), vencido); await launchUrl(Uri.parse('https://wa.me/$wa?text=${Uri.encodeComponent(txt)}'), mode: LaunchMode.externalApplication); return false; }, child: AlumnoCard(a:a, onTap: ()=>Navigator.push(context, MaterialPageRoute(builder:(_)=>AlumnoDetailScreen(alumno:a)))));}),
      ]),
