  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: CustomScrollView(slivers: [
        SliverAppBar(pinned: true, expandedHeight: 140, flexibleSpace: FlexibleSpaceBar(title: Row(mainAxisSize: MainAxisSize.min, children: [Image.asset('assets/logo.png', height: 28, errorBuilder: (_,__,___)=>const Icon(Icons.sports_mma)), const SizedBox(width:8), const Text('Central Boxing')]), background: Container(decoration: const BoxDecoration(gradient: LinearGradient(colors:[Colors.black, Color(0xFFD32F2F)], begin:Alignment.topLeft, end:Alignment.bottomRight)))), actions: [IconButton(icon: const Icon(Icons.settings), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsScreen())))]),
        ...
        SliverToBoxAdapter(child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Padding(padding: const EdgeInsets.symmetric(horizontal:8), child: SegmentedButton<String>(segments: const [ButtonSegment(value:'todos', label:Text('Todos')), ButtonSegment(value:'al_dia', label:Text('Al día')), ButtonSegment(value:'por_vencer', label:Text('Por vencer')), ButtonSegment(value:'vencido', label:Text('Vencidos'))], selected:{p.filtro}, onSelectionChanged:(s){p.filtro=s.first; p.load();})))),
