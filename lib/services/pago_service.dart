...
class PagoService {
  ...
  static Future<void> registrarPago(int alumnoId, double monto, {String metodo='efectivo'}) async {
    final db = await DatabaseHelper.instance.db;
    final maps = await db.query('alumnos', where:'id=?', whereArgs:[alumnoId]);
    ...
    await db.insert('pagos', Pago(alumnoId: alumnoId, fechaPago: DateTime.now(), mesCubierto: mes, monto: monto, metodo: metodo).toMap()..remove('id'));
    await db.update('alumnos', {'fecha_vencimiento': nuevo.toIso8601String()}, where:'id=?', whereArgs:[alumnoId]);
  }
  static Future<List<Pago>> historial(int alumnoId) async {
    final db = await DatabaseHelper.instance.db;
    final maps = await db.query('pagos', where:'alumno_id=?', whereArgs:[alumnoId], orderBy:'fecha_pago DESC');
